...
import {type CreateClassRequest } from '../../types/course.types';
import { useImportSubjects } from '../../hooks/features/useSubjects';
...
const CreateClassModal = ({ isOpen, onClose }: Props) => {
  ...
  const { data: subjectData } = useImportSubjects();
  ...
  const onSubmit = (data: CreateClassRequest) => {
    // Convert string sang number cho subjectId
    const payload = { ...data, subjectId: Number(data.subjectId) };
